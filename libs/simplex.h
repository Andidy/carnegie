...
float noise(double x, double y)
{
  ...
  // calculate the hashed gradient indexes of the corners
  ...
  // Add contributions to get the final noise level
  // Unscaled range [-1, 1]
  double unscaled = 70.0f * (n0 + n1 + n2);
  
  // Scale result to [0, 1]
  return (float)((unscaled + 1.0) * 0.5);
}
