...
i32 LoadImageFromDisk(char* filename, Image* imageData)
{
  ...
  // reset decoder, frame and converter they are per image load structures
  ...
  hr = wicFactory->CreateDecoderFromFilename(
    fuck_windows,                        // Image we want to load in
  ...
  hr = wicFrame->GetSize(&(imageData->width), &(imageData->height));
  if (FAILED(hr)) return 0;
  ...
  imageData->bytesPerRow = (imageData->width * bitsPerPixel) / 8;
  imageData->size = imageData->bytesPerRow * imageData->height;
...
void UploadTextureFromImage(Image* imageData, i32 index, ID3D12Resource** textureBuffer, ID3D12Resource** textureBufferUploadHeap, ID3D12DescriptorHeap** mainDescHeap, ID3D12Device* _device, ID3D12GraphicsCommandList* _commandList)
{
  ...
  HRESULT hresult = _device->CreateCommittedResource(
  ...
  hresult = _device->CreateCommittedResource(
